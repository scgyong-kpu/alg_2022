  ...
  def start(self, start_index=None):
    ...
    while self.weights:
      ...
      for adj, weight in adjacents.items():
        ...
        updated_weight = w + weight
        if adj in self.weights and updated_weight >= self.weights[adj]: continue
        self.weights[adj] = updated_weight
        self.connects[adj] = v
        vis.graph_update_weight(adj, updated_weight)
...
if __name__ == '__main__':
  ...
  # print(cities)
  vis.init('Shortest Path - Dijkstra Algorithm')
  vis.speed = 2

  dijkstra = Dijkstra(cities, edges)
  vis.dijkstra_init(dijkstra)
  dijkstra.start()
  vis.end()
